...
public class VenousSinus extends ElasticTube {
	...
	private float getAddMomentumEquation(Variable pr, Variable fo){
		// equ(72)
		return (pr.getValue() - ModelSpecification.P_OUT[(int) ModelSpecification.currentIter.getValue()]) - ModelSpecification.TPout_alfa.getValue() * fo.getValue();
	}

	private String getSymbolicAddMomentumEquation(Variable pr, Variable fo){
		// equ(72)
		return "("+pr.getName()+" - "+ModelSpecification.P_OUT_NAME+"("+ModelSpecification.currentIter.getName()+")) - "+ModelSpecification.TPout_alfa.getName()+" * "+fo.getName();
	}
