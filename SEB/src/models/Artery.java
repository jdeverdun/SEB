...
public class Artery extends ElasticTube {
	...
	private float getMomentumEquation(SimpleVariable fi, SimpleVariable ar, SimpleVariable pr){
		// equ(31) et equ(36)
		return ModelSpecification.damp2.getValue() * ((fi.getValue()/ar.getValue()) - (getFlowin().getValue()/getArea().getValue()))/ModelSpecification.dt.getValue() + (ModelSpecification.P_INIT.getValue()[(int) ModelSpecification.currentIter.getValue()] - pr.getValue())-getAlpha().getValue()*fi.getValue();
	}
